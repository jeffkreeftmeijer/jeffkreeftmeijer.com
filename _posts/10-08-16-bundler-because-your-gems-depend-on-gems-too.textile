...
But what about gems? We use the @.gemspec@ file to specify gem dependencies for our gems, which -- when complete -- makes installing and using gems a breeze since it installs all dependencies automatically. Great.
...
Now the only place we specify dependencies is in the @.gemspec@, but Bundler can install everything for us making our gem and it's dependencies easy to install for users and contributors. 

Oh, now you have a @Gemfile@ in your project, here's a little bonus: You can use ["Envy Labs":http://envylabs.com/]' "Ruby Tracker":http://rubytracker.com/ by  to get notified when new versions of your dependencies get released. so you can make sure your gem keeps working with the latest stuff. 
