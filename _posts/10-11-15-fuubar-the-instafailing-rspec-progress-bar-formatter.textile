...
excerpt: Merging the work of Nick Evans and Michael Grosser in an attempt to create a better RSpec formatter
published: true
---
...
Besides that, I realized there were more things that could be improved on, like knowing the total amount of specs, the amount of specs that have run already and maybe even an ETA or something like that. Also, the big string of dots and "F"'s started to seem unnecessary, there should be a be a nicer way to display this information. 

I checked out RSpec's formatters and realized it was extremely to write one, so I got distracted and wrote "FffuuuFormatter":http://gist.github.com/676219, which makes RSpec print FFFUUU instead of FFFFF. Heh.
...
I decided to get this formatter up to speed with RSpec 2 and continue Nick's work. I put the whole thing in a gem and "@josevalim(José Valim)":http://twitter.com/josevalim came up with the name: "Fuubar":http://github.com/jeffkreeftmeijer/fuubar.
...
Fuubar only works on RSpec 2 right now, but there's no reason why it can't work on earlier versions. Also, Nick's original code had some functionality to find slow specs, but I omitted it because I couldn't really find a nice implementation.
