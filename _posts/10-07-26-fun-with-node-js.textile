...
<div class="notice" id="no_web_sockets">
  For this to blow your mind, you need a browser that supports web sockets. May I suggest one of the newer versions of "Chrome":http://www.google.com/chrome/ or "Safari":http://www.apple.com/safari/?
</div>
...
Starting the server is as simple as running @node server.js@. To make sure it keeps running, I daemonized it with "God":http://god.rubyforge.org/, using the same config file I used in the "&ldquo;Daemonizing Navvy with God&rdquo; article":http://jeffkreeftmeijer.com/2010/daemonizing-navvy-with-god/.

h3. Receiving messages

Now, in a regular javascript file I included into the page, I connect to the web socket like this:
...
Now we're able to receive messages, move and delete cursors. The last thing we need is the client to send out messages:
...
Tracking mouse movement and showing cursors to other clients is cool, but not useful in any way. What this example does show is that you can do pretty impressive things using web sockets and Node.js. And it was a great excuse to play around with it.

This was the first thing I did using Node.js, so the code is probably far from perfect. If you know a way to improve it, please "fork the Gist":http://gist.github.com/488562 and show me how it should be done.
