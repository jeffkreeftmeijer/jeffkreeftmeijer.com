...
---
  ...
  Last week, I published an article about "my very first experiment":http://jeffkreeftmeijer.com/2010/experimenting-with-node-js with "Node.js":http://nodejs.org. Being completely new to the whole thing, I decided to keep it simple and something stupid with web sockets.

As you can read in last week's article, I created a page that checks your mouse cursor's location and sends it to the web socket. The web socket broadcasts your mouse location to every other user on the page, so everyone will see your cursor moving around.
...
Every successful experiment needs results and I got some, so I decided to improve the code a bit. In this article I'll go over some things I learned and how I fixed some issues.

Oh, and I've updated "the Gist":http://gist.github.com/488562, in case you want to use it. 

h3. People love fancy tricks with web sockets

After posting the article, it jumped to the "Hacker News":http://news.ycombinator.com/item?id=1548321 front page within minutes and stayed there for over 26 hours, it got "retweeted like crazy":http://tweetmeme.com/story/1782313910/experimenting-with-nodejs-jeff-kreeftmeijer and it got featured on "episode #17 of the Dev show":http://5by5.tv/devshow/17.
...
Since this was my first experiment with Node.js, I expected cursors to start moving slow and the server eventually crashing. Especially because of this unexpected stream of visitors.
...
But, except for some mistakes I made which caused the server to crash when somebody injected some javascript, Node.js handled everything perfectly. On a 256 MB VPS. Amazing.
...
Right now, the example runs on ["@rauchg(Guillermo Rauch)":http://twitter.com/rauchg]'s "Socket.IO":http://github.com/LearnBoost/Socket.IO-node instead of "web-socket-server":http://github.com/miksago/node-websocket-server, which automatically falls back to flash when your browser doesn't support web sockets.
...
Since I needed to send cursor locations on @mousemove()@, I had to create a connection to the web socket using Javascipt. This made it quite easy to send anything you want by injecting some Javascript into the page and calling @conn.send@ and the web socket server would simply broadcast it to the other clients.

I was amazed to see "@einaros(Einar Otto Stangvik)":http://twitter.com/einaros found himself able to inject multiple cursors into the page and rotate them in 3D space. He even made "a video":http://www.youtube.com/watch?v=jULOA7mSOac.
