...
"Lighthouse":http://lighthouseapp.com is the tracking solution that's being used for "the Ruby on Rails project":rails.lighthouseapp.com. While it works most of the time, sometimes it seems like it's not built for projects this big and the amount of tickets and updates can seem overwhelming. In this article I'll try to explain my way of keeping up, which has been working for me over the last few weeks.

!http://jeffkreeftmeijer.com/images/lighthouse.png!

Lighthouse provides an "RSS feed for project events":https://rails.lighthouseapp.com/projects/8994-ruby-on-rails/events.atom so you immediately know when any ticket gets updated, closed, when a new one gets created and all sorts of other stuff. While that might sound like the perfect solution, this feed is practically useless for a project as big and active as Rails because there's too much going on to keep up with.

It didn't take me long unsubscribe from that fire hose and decide to change my strategy. I ended up watching tickets that seemed interesting and setting up email notifications. This way, Lighthouse sends me an email whenever one of my watched tickets get updated. I have a filter in Gmail to catch everything coming from Lighthouse, label it with &ldquo;rails&rdquo; and skip my inbox. Now and again, I'll go through the messages with this label and see if I can do anything to help out.

Whenever you comment on a ticket, Lighthouse will automatically add you to the watch list so you'll receive email for everything you're involved with.
...
I went over the "&ldquo;this week&rdquo;-bin":https://rails.Lighthouseapp.com/projects/8994-ruby-on-rails/tickets/bins/31228 every once in a while to find tickets to watch or write a patch for. Because there's no good way to see if you've already read a specific ticket, this turned out to be quite a task.
...
Using "Lighthouse Keeper":http://keeper.jeffkreeftmeijer.com, I'm subscribed to a feed that shows me tickets ordered by ticket number (`@order:number-@`) now. This means new tickets automatically end up in my feed reader. Whenever something interesting comes along, I'll comment on the ticket, watch it, or assign it to myself if I feel confident enough to write a patch.

Needless to say, I would _love_ Lighthouse to provide RSS feeds like this.
